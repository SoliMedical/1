...
**الإصدار المرجعي:** v1.6.1
**الحالة:** بُنيت طبقة الترحيل والتخزين والأرشفة والكتابة المزدوجة في التطبيق، لكنها لا تبدأ النقل من تلقاء نفسها. تظل الوثيقة القديمة هي مصدر القراءة التشغيلي إلى أن ينفذ مدير العيادة معاينة ثم يوافق صراحة على إنشاء نسخة V2 ومراجعة تقرير المطابقة.
...
## حالة التنفيذ في v1.6.1

| المكوّن | ما نُفذ | ما لا يزال متوقفاً لحماية البيانات |
|---|---|---|
| Firebase Storage | أضيفت مكتبة Compat، ويرفع الشعار إلى `clinics/{clinicId}/branding/*` ويحفظ الرابط والمسار فقط | لا يُحذف كائن الشعار السابق تلقائياً، ولا يبدأ تحويل شعار Base64 دون تأكيد المدير |
| ترحيل V2 | تُنشأ وثائق مستقلة للمرضى والزيارات والمواعيد والفواتير والروشتات والمصروفات والإعدادات والقوائم وسجل التدقيق وبيانات الأرشيف في دفعات من 400 | لا يُشغّل من دون اتصال أو حساب مدير أو نافذة تأكيد؛ ولا يحذف الوثيقة القديمة |
| قابلية الاستئناف | يسجل التطبيق `migrationId` و`migrationManifests/{migrationId}` ويستخدم `set(..., { merge: true })` | لا يحوّل قراءة التطبيق اليومية إلى V2 تلقائياً قبل مقارنة الأعداد والمعرفات |
| الكتابة المزدوجة | بعد إكمال نسخة V2، تُحفظ الوثيقة القديمة أولاً ثم تُرسل التغييرات المتبدلة فقط إلى V2 عبر بصمات محلية ودفعات صغيرة | لا تحول هذه الكتابة مسار القراءة إلى V2، وتؤجل المحاولة عند غياب الاتصال أو المصادقة |
| الأرشفة | يمكن أرشفة عرض المواعيد القديمة واستعادتها، ويُحفظ `archiveManifests` قابل للتدقيق لكل عملية أرشفة أو استعادة، وتبقى الزيارة والموعد المكتمل محفوظين | لا يوجد حذف تلقائي للمرضى أو الزيارات أو الروشتات أو قائمة الانتظار |
| تقرير المطابقة | يقارن المدير أعداد الوثائق المتوقعة مع مجموعات V2 بما فيها `auditLogs` و`archiveManifests` قبل أي تحويل مستقبلي للقراءة | لا يسمح التطبيق بتحويل القراءة تلقائياً حتى عند نجاح التقرير |

> بعد إنشاء نسخة V2، يبقى التطبيق في وضع **القراءة القديمة مع كتابة مزدوجة محمية**. هذه خطوة مقصودة؛ التحويل النهائي للقراءة يتطلب تقرير مطابقة مفصل واختباراً دون اتصال على نسخة اختبارية أولاً.
...
## 9. نقاط قرار قبل تفعيل القراءة من V2 أو تنفيذ ترحيل إنتاجي
