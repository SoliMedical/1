...
describe("صفحة الحجز الذاتي عبر واتساب", () => {
  ...
  it("يعرض رقم إصدار واضحاً يطابق النسخة المنشورة الحالية", () => {
    expect(page).toContain("const SOLI_APP_VERSION = 'v1.6.4'");
  ...
  it("يرفع نسخة Service Worker عند تغييرات التطبيق حتى لا تبقى نسخة مواعيد قديمة", () => {
    expect(page).toContain("navigator.serviceWorker.register('./sw.js?v=soli-v1.6.4', { updateViaCache: 'none' })");
    expect(sw).toContain('soli-medical-pwa-v15');
